...
class IPC(nn.Module): 
	...
	meanfielddim = action_dim
        ...
        self.meanfield_dim = meanfielddim 
        ...
        self.critics.add_module('critic_fc1', nn.Linear(2 * hidden_dim,
                                                  hidden_dim))
        ...
        if norm_in:
            self.state_encoders.add_module('s_enc_bn', nn.BatchNorm1d(
                                        idim, affine=False))
        self.state_encoders.add_module('s_enc_fc1', nn.Linear(idim,
                                                        hidden_dim))
    ...
    def forward(self, inps, state_agent, action_agent, meanfield_agent, return_q=True, return_all_q=False,
        ...
        """
        agents = range(1)
        states = [s for s, meanfield, a in inps]
        meanfield = [meanfield for s,meanfield,a in inps]
        actions = [a for s,meanfield,a in inps]

        list_state = [[] for i in range(len(states[0]))] 
        list_action = [[] for i in range(len(states[0]))] 
        list_meanfield = [[] for i in range(len(states[0]))] 

        for i in range(len(states[0])):
            for j in range(len(states)):
                list_state[i].append(states[j][i])
                list_meanfield[i].append(meanfield[j][i])
        ...
        for i in range(len(list_state)):
            b = torch.Tensor(self.batch_size, self.state_dim, self.action_dim)
            list_state[i] = torch.cat(list_state[i], out=b)
            b = torch.Tensor(self.batch_size, self.action_dim)
            list_action[i] = torch.cat(list_action[i], out=b)
            b = torch.Tensor(self.batch_size, self.action_dim)
            list_meanfield[i] = torch.cat(list_meanfield[i], out=b)

        for i in range(len(list_action)):
            list_action[i] = list_action[i].reshape(self.batch_size, self.action_dim)


        new_inps = [torch.cat((s.reshape(self.batch_size, self.state_dim, self.action_dim, self.meanfield_dim), a.reshape(self.batch_size, self.action_dim)), dim=1) for s, a in zip(list_state, list_action)]
        own_inps = torch.cat((state_agent, action_agent), dim=1)
        sa_encodings = [encoder(inp) for encoder, inp in zip(self.critic_encoders, new_inps)]
        s_encodings = [self.state_encoders(own_inps) for a_i in agents]
        ...
        for i, a_i in enumerate(agents):
            ...
            int_acs = action_agent.max(dim=1, keepdim=True)[1]
...
class CriticNet(nn.Module):
    def __init__(self, state_dim, action_dim, h_dim = 200):
        ...
        self.action_dim = action_dim 
        self.h_dim = h_dim 
        self.fc = nn.Sequential(nn.Linear(self.state_dim + self.action_dim +self.meanfield_dim, self.h_dim),
                                nn.ReLU6(),
                                nn.Linear(self.h_dim, self.h_dim),
                                nn.ReLU6(),
                                nn.Linear(self.h_dim, self.h_dim))
...
class Agent():
    ...
    def __init__(self, state_dim, action_dim, n_agents, h_dim = 200, epsilon = 0.9, LR_A = 1e-4, LR_C = 1e-4, gamma = 0.9, replace_iter_a = 1100, replace_iter_c = 1000, memory_capacity = 20000, batch_size = 64):
        ...
        self.h_dim = h_dim
        self.eval_cnet, self.target_cnet = IPC(self.state_dim, self.meanfielddim, self.action_dim, self.n_agents, batch_size, hidden_dim=self.h_dim).float(), IPC(self.state_dim, self.action_dim, self.n_agents, batch_size, hidden_dim=self.h_dim).float()
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            target_value = self.target_cnet(merged_list2, s_, action)
            q_target = r + self.gamma * target_value  
        
        q_eval = self.eval_cnet(merged_list, s, a)
        ...
        a_loss = -(log_policy * target_value).mean()
        a_loss.backward()
