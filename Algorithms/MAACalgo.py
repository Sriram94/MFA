...
class AttentionCritic(nn.Module): 
    ...
    def forward(self, inps, state_agent, action_agent, return_q=True, return_all_q=False,
...
class Agent():
    ...
    def __init__(self, state_dim, action_dim, n_agents, h_dim = 50, epsilon = 0.9, LR_A = 1e-4, LR_C = 1e-2, gamma = 0.9, replace_iter_a = 10, replace_iter_c = 10, memory_capacity = 20000, batch_size = 64):
        ...
        self.h_dim = h_dim
        self.eval_cnet, self.target_cnet = AttentionCritic(self.state_dim, self.action_dim, self.n_agents, batch_size, hidden_dim=self.h_dim).float(), AttentionCritic(self.state_dim, self.action_dim, self.n_agents, batch_size, hidden_dim=self.h_dim).float()
    ...
    def save_param(self, path='./param/maac.pkl'):
        print("Saving")
        save_dict = {'networks': [network.state_dict() for network in self.networks]}
        torch.save(save_dict, path)

    def load_param(self, path='./param/maac.pkl'):
        print("loading")
