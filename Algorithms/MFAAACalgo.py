...
class MFAAAC(nn.Module): 
    def __init__(self, state_dim, action_dim, num_elements, batch_size, hidden_dim=50, norm_in=False, attend_heads=4):
        super(MFAAAC, self).__init__()
...
class Agent():
    ...
    def __init__(self, state_dim, action_dim, num_elements, h_dim = 50, epsilon = 0.9, LR_A = 1e-4, LR_C = 1e-2, gamma = 0.9, replace_iter_a = 10, replace_iter_c = 10, memory_capacity = 20000, batch_size = 64, temperature=0.1):
        ...
        self.h_dim = h_dim
        self.eval_cnet, self.target_cnet = MFAAAC(self.state_dim, self.action_dim, self.num_elements, batch_size, hidden_dim=self.h_dim).float(), MFAAAC(self.state_dim, self.action_dim, self.num_elements, batch_size, hidden_dim=self.h_dim).float()
    ...
    def update(self):
        ...
        with torch.no_grad():
            q_values = self.target_cnet(merged_list2, s_, meanfield)
            max_value = torch.max(q_values)
            q_target = r + self.gamma * max_value
        
        
        q_eval = self.eval_cnet(merged_list, s, meanfield)
