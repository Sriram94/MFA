...
class MFAAQL(nn.Module): 
    def __init__(self, state_dim, action_dim, n_elements, batch_size, hidden_dim=200, norm_in=False, attend_heads=4):
        super(MFAAQL, self).__init__()
        ...
        self.critics.add_module('critic_fc1', nn.Linear(3 * hidden_dim,
                                                  hidden_dim))
    ...
    def forward(self, inps, state_agent, meanfield, return_q=False, return_all_q=True,
        ...
        if tmp != 1:  
            ...
            for i in range(len(list_state)):
                b = torch.Tensor(self.batch_size, self.state_dim)
            ...
            new_inps = [torch.cat((s.reshape(self.batch_size, self.state_dim), a.reshape(self.batch_size, self.action_dim)), dim=1) for s, a in zip(list_state, list_action)]
            ...
            new_inps = [torch.cat((s.reshape(tmp, self.state_dim), a.reshape(tmp, self.action_dim)), dim=1) for s, a in zip(list_state, list_action)]
        ...
        for i, a_i in enumerate(agents):
            ...
            agent_rets = []
            critic_in = torch.cat((s_encodings[i], meanfield_encodings[i], *other_all_values[i]), dim=1)
...
class Agent():
    ...
    def __init__(self, state_dim, action_dim, n_elements, h_dim = 200, epsilon = 0.9, LR_A = 1e-4, LR_C = 1e-4, gamma = 0.9, replace_iter = 1100, memory_capacity = 20000, batch_size = 64, temperature=0.1):
        ...
        self.h_dim = h_dim
        self.eval_net, self.target_net = MFAAQL(self.state_dim, self.action_dim, self.n_elements, batch_size, hidden_dim=self.h_dim).float(), MFAAQL(self.state_dim, self.action_dim, self.n_elements, batch_size, hidden_dim=self.h_dim).float()
    ...
    def select_action(self, state, sothers, aothers, meanfield):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            q_value = self.eval_net(merged_list, state, meanfield)
    ...
    def update(self):
        ...
        with torch.no_grad():
            q_values = self.target_net(merged_list2, s_, meanfield)
            max_value = torch.max(q_values)
            q_target = r + self.gamma * max_value 
        
        
        q_eval = self.eval_net(merged_list, s, meanfield)
    ...
    def save_param(self, path='./param/mfaaql.pkl'):
        print("Saving")
        save_dict = {'networks': [network.state_dict() for network in self.networks]}
        torch.save(save_dict, path)

    def load_param(self, path='./param/mfaaql.pkl'):
        print("loading")
