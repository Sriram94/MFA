...
class CriticNet(nn.Module):
    def __init__(self, state_dim, action_dim, h_dim = 50):
        ...
        self.fc1 = nn.Linear(self.state_dim+1, h_dim)
        self.fcm1 = nn.Linear(self.action_dim, h_dim)

        self.fc = nn.Sequential(nn.Linear(self.h_dim*2, self.h_dim),
                                nn.ReLU6(),
                                nn.Linear(self.h_dim, self.h_dim),
                                nn.ReLU6(),
                                nn.Linear(self.h_dim, self.h_dim))
        ...
        self.fcm1.apply(init_weights)
    
    def forward(self, sa, mean_a):
        x = self.fc1(sa)
        x = F.relu(x)
        y = self.fcm1(mean_a)
        y = F.relu(y)
...
class Agent():
    ...
    def __init__(self, state_dim, action_dim, h_dim = 50, epsilon = 0.9, LR_A = 1e-4, LR_C = 1e-2, gamma = 0.9, replace_iter_a = 10, replace_iter_c = 10, memory_capacity = 20000, batch_size = 64, temperature=0.1):
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            q_target = r + self.gamma * self.target_cnet(torch.cat((s_, action), dim=1), meanfield)
        
        a = torch.unsqueeze(a, dim=-1)
        q_eval = self.eval_cnet(torch.cat((s, a), dim=1), meanfield)
        ...
        a_loss = -(log_policy * self.eval_cnet(torch.cat((s, action), dim=1), meanfield)).mean()
        a_loss.backward()
    ...
    def save_param(self, path='./param/mfac.pkl'):
        print("Saving")
        save_dict = {'networks': [network.state_dict() for network in self.networks]}
        torch.save(save_dict, path)

    def load_param(self, path='./param/mfac.pkl'):
        print("loading")
