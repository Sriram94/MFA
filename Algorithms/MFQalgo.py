...
class Net(nn.Module):
    def __init__(self, n_states, n_actions):
        ...
        self.fcm1 = nn.Linear(n_actions, 50)
...
class MFQ(object):
    def __init__(self, n_states, n_actions, batch_size=64, lr=0.01, epsilon=0.9, gamma = 0.9, target_replace = 10, memory_capacity=20000):
        ...
        self.memory = np.zeros((memory_capacity, n_states * 2 + 2 + n_actions))     
        self.optimizer = torch.optim.Adam(self.eval_net.parameters(), lr=self.lr)
        self.loss_func = nn.MSELoss()

    def choose_action(self, x, meanfield):
        
        x = torch.unsqueeze(torch.FloatTensor(x), 0).to(device)
        meanfield = torch.FloatTensor(meanfield).to(device)

            
        if np.random.uniform() < self.epsilon:   
            actions_value = self.eval_net.forward(x, meanfield).to(device)
    ...
    def store_transition(self, s, a, r, meanfield, s_):
        meanfield = meanfield[0]
        transition = np.hstack((s, [a, r], meanfield, s_))
    ...
    def learn(self):
        ...
        b_mean = torch.FloatTensor(b_memory[:, self.n_states+2:self.n_states+2+self.n_actions]).to(device)
        b_s_ = torch.FloatTensor(b_memory[:, -self.n_states:]).to(device)



        q_eval = self.eval_net(b_s, b_mean).gather(1, b_a)  
        q_next = self.target_net(b_s_, b_mean).detach()     
