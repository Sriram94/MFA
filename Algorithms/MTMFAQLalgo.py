...
class Agent():
    ...
    def __init__(self, state_dim, action_dim, action_dim2, n_elements, observation_space, action_space, h_dim = 200, epsilon = 0.9, LR_A = 1e-2, LR_C = 1e-4, gamma = 0.9, replace_iter = 10, memory_capacity = 20000, batch_size = 64, temperature=0.1):
        ...
        self.h_dim = h_dim
        self.eval_net, self.target_net = MTMFAQL(self.state_dim, self.action_dim, self.action_dim2, self.n_elements, batch_size, observation_space, action_space, hidden_dim=self.h_dim).float(), MTMFAQL(self.state_dim, self.action_dim, self.action_dim2, self.n_elements, batch_size, observation_space, action_space, hidden_dim=self.h_dim).float()
    ...
    def save_param(self, path='./param/mtmfaql.pkl'):
        print("Saving")
        save_dict = {'networks': [network.state_dict() for network in self.networks]}
        torch.save(save_dict, path)

    def load_param(self, path='./param/mtmfaql.pkl'):
        print("loading")
