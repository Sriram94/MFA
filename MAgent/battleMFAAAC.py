...
def run_battle(parallel_env):
    ...
    with open('pettingzoomagentmfaaac.csv', 'w+') as myfile:
        myfile.write('{0},{1},{2}\n'.format("Episode", "sumofrewards(MFAAAC)", "sumofrewards(MFAAAC)"))
    ...
    while num_episode < 5:
            ...
            for agent in parallel_env.agents:
                ...
                if "red" in agent: 
                    if (np.random.uniform() <= eps):
                        action = dqn1.choose_action(agent_observation)
                    else:
                    ...
                    if (np.random.uniform() <= eps):
                        action = dqn2.choose_action(agent_observation)
                    else:
        ...
        with open('pettingzoomagentmfaaac.csv', 'a') as myfile:
...
if __name__ == "__main__":
    ...
    dqn1 = DQN(6929, 21, epsilon = 1)
    dqn2 = DQN(6929, 21, epsilon = 1)
    dqn1.restore("./dqnmodel1.ckpt")
    dqn2.restore("./dqnmodel2.ckpt")
