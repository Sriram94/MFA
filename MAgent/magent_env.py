...
class magent_parallel_env(ParallelEnv):
    def __init__(self, env, active_handles, names, map_size, max_cycles, reward_range, minimap_mode, extra_features):
        ...
        action_spaces_list = [Discrete(num_actions[j]) for j in range(len(self.team_sizes)) for i in range(self.team_sizes[j])]
        ...
        observation_space_list = [Box(low=0., high=2., shape=team_obs_shapes[j], dtype=np.float32) for j in range(len(self.team_sizes)) for i in range(self.team_sizes[j])]
    ...
    def get_neighbour_list(self, actions, elements):
        ...
        for i in range(len(pos_list)):
            neighbour_list = self.env.get_neighbors(i, pos_list)
    ...
    def get_type_neighbour_list(self, actions, elements):
        ...
        for i in range(len(pos_list)):
            ...
            sum_list = 0
            for j in range(len(neighbour_list)):
                if "mele" in action_list[neighbour_list[j]]: 
                    action_neighbour_list.append(action_list[neighbour_list[j]])
                    sum_list = sum_list + 1
                if sum_list >= elements/2: 
                    break
            neighbour_action1[i] = action_neighbour_list
            
            sum_list = 0
            for j in range(len(neighbour_list)):
                if "ranged" in action_list[neighbour_list[j]]: 
                    action_neighbour_list.append(action_list[neighbour_list[j]])
                    sum_list = sum_list + 1
                if sum_list >= elements/2: 
                    break
            neighbour_action2[i] = action_neighbour_list

        


        new_neighbour_action1 = {}
        new_neighbour_action2 = {}
        
        i = 0
        for key in actions:
            new_neighbour_action1[key] = neighbour_action1[i]
            new_neighbour_action2[key] = neighbour_action2[i]
            i = i + 1 

        return new_neighbour_action1, new_neighbour_action2
        
    def get_neighbour_tiger_list(self, actions, elements):
