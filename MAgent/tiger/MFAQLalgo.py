...
class Agent():
    ...
    def __init__(self, state_dim, action_dim, n_elements, h_dim = 200, epsilon = 0.9, LR_A = 1e-4, LR_C = 1e-4, gamma = 0.9, replace_iter = 1100, memory_capacity = 20000, batch_size = 64, temperature=0.1):
        ...
        self.h_dim = h_dim
        self.eval_net, self.target_net = MFAQL(self.state_dim, self.action_dim, self.n_elements, batch_size, hidden_dim=self.h_dim).float(), MFAQL(self.state_dim, self.action_dim, self.n_elements, batch_size, hidden_dim=self.h_dim).float()
    ...
    def save_param(self, path='./param/mfaql.pkl'):
        print("Saving")
        save_dict = {'networks': [network.state_dict() for network in self.networks]}
        torch.save(save_dict, path)

    def load_param(self, path='./param/mfaql.pkl'):
        print("loading")
